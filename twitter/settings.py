...
SECRET_KEY = '_0z$jcav#@(tbaz+&51&_#1oi8c%g7_qkef1+e(@5xb+&oq^n&'
...
# redis machine no.1 is used for cache, so here we use machine no.2 for message broker
CELERY_BROKER_URL = 'redis://127.0.0.1:6379/2' if not TESTING else 'redis://127.0.0.1:6379/0'
CELERY_TIMEZONE = "UTC"
# when testing, we don't do the tasks asynchronously
...
RATELIMIT_CACHE_PREFIX = 'rl:'  # in case it conflicts with other keys
RATELIMIT_ENABLE = not TESTING  # in internal testing or dev env, we usually disable it
