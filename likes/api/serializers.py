...
class LikeSerializerForCancel(BaseLikeSerializerForCreateAndCancel):
    def cancel(self):
        """
        unlike create(), which can be triggered by serializer.save(),
        cancel() is a method that we define by ourselves, so to trigger this one,
        we need to call serializer.cancel()
        """
