...
class HBaseModel:
    ...
    def save(self):
        ...
        # if row_data is empty, meaning we have no column key values to store,
        # hbase would not store the row_key, so we need to raise an exception to notify caller
        # to avoid storing empty values
        if len(row_data) == 0:
    ...
    def serialize_field(cls, field, value):
        ...
        if isinstance(field, IntegerField):
            # it is sorting by dict sequence, so there might be sequence like 1, 10, 2
            # to solve this, we fix the length of the int to be 16 bit (times of 8 utilizes space better)
            # if the int does not have enough length, we add 0
            value = str(value)
            while len(value) < 16:
                value = '0' + value
        if field.reverse:
    ...
    def deserialize_row_key(cls, row_key):
        ...
        # val1:val2 => val1:val2: this is easier to find a val everytime we search for (':')
        row_key = row_key + ':'
